...
router.post('/save-draft', upload, async (req, res) => {
    const {
        ...
        author_id,
        modules,
        tags, // Переконуємося, що теги також приходять
    } = req.body;
    ...
    if (tags && typeof tags === 'string') {
        parsedTags = tags.split(',').map(tag => tag.trim()); // Якщо це рядок, перетворюємо в масив
    }
    ...
    try {
        ...
        if (modules && modules !== 'undefined' && modules !== null) {
            let modulesArray = [];
            try {
                modulesArray = JSON.parse(modules); // Перевірка на валідний JSON
            } catch (err) {
        ...
        if (parsedTags && Array.isArray(parsedTags)) {
            const insertTagsQuery = `
                ...
                RETURNING id;
            `;
            const tagIds = await pool.query(insertTagsQuery, parsedTags);
...
router.post('/create', upload, async (req, res) => {
    const {
        ...
        author_id,
        modules,
        status, // статус (draft або published)
    } = req.body;
    ...
    try {
        ...
        const courseStatus = status || 'draft'; // Якщо статус не вказано, буде чернетка
